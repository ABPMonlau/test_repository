...
public class reserveController {
    ...
    public ResponseEntity<clients> addClient(@RequestBody clients c) {
        ...
        c.setId(clientId);
        System.out.println('Done! ID: ' + clientId);
    ...
    @PostMapping('/buscar/libres')
    public ResponseEntity<?> checkReserve(@RequestBody books b) {
        System.out.print('Checking for tables...');
        timeShiftStates shift = timeShiftStates.comida;
        if (b.getShift() != null && b.getShift().getTimeShift() != null) {
            try {
                shift = timeShiftStates.valueOf(b.getShift().getTimeShift());
            } catch (Exception e) {
                System.out.println('Invalid shift name, defaulting to comida');
            }
        }
        List<tables> tableList = tableDAO.getAvaliableTables(b.getGuests(), shift);
        if (tableList.isEmpty()) {
            System.out.println('No available tables found for ' + shift + '!');
